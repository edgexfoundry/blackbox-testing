...
docker-compose -f ${docker_compose_test_tools} run --rm postman run ${COLLECTION_PATH} --folder="subscription" \
  --iteration-data="data/subscriptionData.json"  --environment=${ENV_PATH} --reporters="junit,cli" \
  --insecure --global-var accessToken="$TOKEN"
docker-compose -f ${docker_compose_test_tools} run --rm postman run ${COLLECTION_PATH} --folder="notification" \
  --iteration-data="data/notificationData.json"  --environment=${ENV_PATH} --reporters="junit,cli" \
  --insecure --global-var accessToken="$TOKEN"
docker-compose -f ${docker_compose_test_tools} run --rm postman run ${COLLECTION_PATH} --folder="transmission" \
  --iteration-data="data/transmissionData.json"  --environment=${ENV_PATH} --reporters="junit,cli" \
  --insecure --global-var accessToken="$TOKEN"
docker-compose -f ${docker_compose_test_tools} run --rm postman run ${COLLECTION_PATH} --folder="ping"  --environment=${ENV_PATH} \
